...
5. print() function = way to communicate with the program (output)
6. assert = used to make verifications
7. input() function = to give data to a program from the user keyboard
...
```
if
if-else = if the condition is true goes on the first branch, if the condition is false goes on the second branch
if-else-if-else
```
Session 3 \
...
for else: runs only when the iteration of for ends without any taking something from for
continue: continues the iteration to the next step
while: iterates until the condition is true
while else: runs only when the iteration of while ends without any taking something from a while
...
self = keyword used to access a variable that belongs to that class
...
Abstraction =  hides all but the relevant data about an object. used as a template for 
    methods from the child class. doesn't have a constructor. can't create abstract objects
Polymorphism = objects take different forms or behaviors in different ways depending on 
...
find_element(By.XPATH, '//*[@id="password"]') - XPATH selector
    // - starts from the root and takes all the descendants
    * - any type of tag
...
```
sleep() - code sleeps x seconds, no matter of element or event from the code. the test 
    will have more time to run
implicit wait - a global setting that applies to all elements in a Selenium script. It waits
    a specified time before throwing an exception if the element is not found
explicit wait - when it waits for a certain condition to occur before proceeding 
...
any number of tests - mandatory all the tests have to start with the keyword 'test'
...
syntax to 'log in' with basic authentication (pop-up from the browser)
```
Session 11 \
...
And, But: for making the connection between: Give, When, and Then
```
Gherkin tests are in the .feature file:
```
...
def step_imp(context):
    pass
```
Object 'context' is sent as a parameter to 'behave' to send information through steps

To install:
1. pip install behave (https://behave.readthedocs.io/en/latest/ library for running the tests from Gherkin)
...
3. pip install behave-html-formatter (https://pypi.org/project/behave-html-formatter/ library to create HTML reports) \
    3.1. It is a must to create a file 'behave.ini' in the main folder on the same level as the feature. Make the settings for this file to create the HTML report. \
    3.2.  To run the code and to generate an HTML report use this command: behave -f html -o behave-report.html
...
POM (Project Object Model) = design pattern used to reduce code duplication and to minimize the effort involved in code maintenance. helps to organize the files in a logical way and is easy to use. Each page of the project will have an individual class where we can find the elements of the page.

Agile methodology = breaking the project into phases and emphasizing continuous collaboration and improvement \
...
```
Epic (Step 1) = functionality of the application (can be divided into tasks)
...
1. Directory: Features
   1. directory: pages = get the selectors from the webpage and implement the functionality of each step
      1. login_page.py
...
To run the code after writing the scenario, steps, and functions use this command line on the terminal:
```command line
...
To run only one single Scenario this line of code can be used:
```command line
behave -f html -o behave-report.html --tags=single
```

Session 13 \
Continue working on a project from Session 12

Session 14 \
Unit Testing = testing to the lowest level possible. The function that tests another function. The test function calls the tested function and returns the right data using assert.

To do a unit test you have to install 'pytest' library using this line on code:
```command line
pip install pytest
```
...
HTTP/HTTPS = hypertext transfer protocol / secure = communication protocol between client and server. helps to transfer data over the network

HTTP Answers: https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#client_error_responses
```command line
200 = OK success = when request data from the server
...
401 = Unauthorized = when is not logged in
403 = Forbidden = when the user is logged in but doesn't have the rights to edit
404 = Not Found = the server can't find the requested resource
408 = Request Timeout = It takes too long to load the request from the server

500 = Internal Server Error = request arrived on server and it is a bug
...
Server listens to receive requests -> processes the information -> sends an answer to client -> shows to the user
...
POST = when sending data to server -> usually 201
